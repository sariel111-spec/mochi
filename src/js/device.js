...
(function () {
  ...
  // 高频交互轨迹延迟到当前事件结束后再落盘，避免同步 localStorage 占用触摸/点击关键路径。
  ...
  function traceRingPush(key, ent, cap) {
    try {
      ...
      // 连续点击/滑动视为一批：每次交互把落盘推迟到 220ms 空闲后，
      // 避免 setTimeout(0) 在每个手势尾部立刻做同步 localStorage 读/解析/写。
      if (_traceRingTimer !== null) clearTimeout(_traceRingTimer);
      _traceRingTimer = setTimeout(flushTraceRings, 220);
    } catch (e) {}
  }
