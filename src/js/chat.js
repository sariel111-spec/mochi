...
if (deskMsgEl) {
// 手机滚动快路径：non-passive touchmove 只在真正拖动桌面消息卡的这一段触摸期间存在。
// 常驻 window passive:false 会让 Safari/Chromium 每次页面滑动都先同步等待 JS，
// 即使 dDrag=false 也会增加滑动/点击后的输入延迟。
const deskMsgTouchMove = (e) => {
if (!dDrag) return;
...
const armDeskMsgTouchMove = () => {
try { window.removeEventListener('touchmove', deskMsgTouchMove, false); } catch (err) {}
try { window.addEventListener('touchmove', deskMsgTouchMove, { passive: false }); } catch (err) {}
};
const disarmDeskMsgTouchMove = () => {
try { window.removeEventListener('touchmove', deskMsgTouchMove, false); } catch (err) {}
};
deskMsgEl.addEventListener('touchstart', (e) => {
const t = e.touches && e.touches[0];
if (t) { deskMsgDragStart(t.clientX, t.clientY); armDeskMsgTouchMove(); }
}, { passive: true });
const endTouch = (e) => {
const c = e.changedTouches && e.changedTouches[0];
deskMsgDragEnd(c ? c.clientX : (dDrag ? dDrag.x : 0));
disarmDeskMsgTouchMove();
};
