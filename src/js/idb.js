...
(function () {
  ...
  // 恢复日志是“额外保险副本”，业务键本身仍在 xyStore.set 里同步写 LS，并同时写 IDB。
  // 重度用户的 __wr-journal 可达 100KB+；旧版每次点开关/按钮触发小键 set 都同步
  // JSON.stringify 整包 + setItem 整包，iOS 上会形成低于 50ms longtask 阈值但可感知的
  // 高频输入延迟。现在只把这份冗余保险日志延迟到用户停手后的空闲期；切后台/pagehide
  // 立即冲刷，实际业务值的同步持久化语义不变。
  let _wrjPersistT = null;
  let _wrjPersistIdle = null;
  function wrjPersistNow() {
    if (_wrjPersistT) { clearTimeout(_wrjPersistT); _wrjPersistT = null; }
    if (_wrjPersistIdle !== null && typeof cancelIdleCallback === 'function') {
      try { cancelIdleCallback(_wrjPersistIdle); } catch (e) {}
      _wrjPersistIdle = null;
    }
    try { localStorage.setItem(WRJ_KEY, JSON.stringify(_wrj || [])); } catch (e) {}
  }
  function wrjPersist() {
    if (_wrjPersistT) clearTimeout(_wrjPersistT);
    _wrjPersistT = setTimeout(function () {
      _wrjPersistT = null;
      try {
        if (typeof requestIdleCallback === 'function') {
          _wrjPersistIdle = requestIdleCallback(function () { _wrjPersistIdle = null; wrjPersistNow(); }, { timeout: 2200 });
        } else {
          wrjPersistNow();
        }
      } catch (e) { wrjPersistNow(); }
    }, 700);
  }
  ...
  try {
    document.addEventListener('visibilitychange', function () {
      try { if (document.visibilityState === 'hidden') { wrjPersistNow(); wrjMarkFlush(); } } catch (e) {}
    });
  } catch (e) {}
  try { if (window.addEventListener) window.addEventListener('pagehide', function () { wrjPersistNow(); wrjMarkFlush(); }); } catch (e) {}
